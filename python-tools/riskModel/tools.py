...
def value_counts(das, nhead=5):
    tmp = pd.value_counts(das).reset_index().rename({"index": das.name},
                                                    axis=1)
    value = pd.DataFrame(['value {}'.format(x + 1) for x in range(nhead)],
                         index=np.arange(nhead)).join(
                             tmp.iloc[:, 0], how="left").set_index(0).T
    freq = pd.DataFrame(['freq {}'.format(x + 1) for x in range(nhead)],
                        index=np.arange(nhead)).join(tmp.iloc[:, 1],
                                                     how="left").set_index(0).T
    nnull = das.isnull().sum()
    freqother = pd.DataFrame(
        {
            das.name: [das.shape[0] - nnull - np.nansum(freq.values), nnull]
        },
...
def get_data_summary(da):
    op = pd.concat([
        pd.DataFrame({
            "type": da.dtypes,
            "n": da.notnull().sum(axis=0)
        }),
        da.describe().T.iloc[:, 1:],
        pd.concat(map(lambda i: value_counts(da.loc[:, i]), da.columns))
    ],
                   axis=1).loc[da.columns]
...
dfp['eventOccurTime2'] = pd.to_datetime(dfp['eventOccurTime2'],
                                        format="%Y-%m-%d")
monthLabel = dfp.groupby(["eventOccurTime2"])["交易流水号"].count().reset_index()
xticks = [
    v for i, v in enumerate(list(monthLabel["eventOccurTime2"])) if i % 20 == 0
]
...
# label按时间分布
labelPdf = dfp.groupby(["eventOccurTime2",
                        "label"])["交易流水号"].count().reset_index()
...
# 客户姓名（测试）
pdf["客户姓名"].value_counts().reset_index().to_csv(
    "./features/accountNameCnts.csv")
...
def getSampleSummary(desc, pdf):
    ...
    return desc, total, label1, round(label1 / total, 4), ruleReject, round(
        ruleReject / total, 4)


lst = [
    getSampleSummary("train", train_pdf),
    getSampleSummary("valid", valid_pdf),
    getSampleSummary("test", test_pdf)
]

colName = [
    "dataName", "totalNum", "label=1Num", "label=1Ratio", "rule=RejectNum",
    "rule=RejectRatio"
]
...
def plotPR(desc, true_y, pred_y):
    ...
    precision, recall, thresholds = precision_recall_curve(true_y,
                                                           pred_y,
                                                           pos_label=1)
    avgPrecision = average_precision_score(true_y,
                                           pred_y,
                                           average="micro",
                                           pos_label=1)
    ...
    plt.xlim([0.5, 1.0])
    plt.title(desc +
              'Precision-Recall curve: AP={0:0.2f}'.format(avgPrecision))
...
def getPRs():
    ...
    avgP1 = average_precision_score(train_y,
                                    pred_train,
                                    average="micro",
                                    pos_label=1)
    p2, r2, _ = precision_recall_curve(valid_y, pred_valid, pos_label=1)
    avgP2 = average_precision_score(valid_y,
                                    pred_valid,
                                    average="micro",
                                    pos_label=1)
    p3, r3, _ = precision_recall_curve(test_y, cat_probs01, pos_label=1)
    avgP3 = average_precision_score(test_y,
                                    cat_probs01,
                                    average="micro",
                                    pos_label=1)
    return [p1, p2, p3], [r1, r2, r3], [
        'train AP={0:0.2f}'.format(avgP1), 'valid AP={0:0.2f}'.format(avgP2),
...
ks_2samp(modelPdf.loc[modelPdf.label == 1, 'prob'],
         modelPdf.loc[modelPdf.label == 0, 'prob'])
...
def plot_lift(y_predproab, y_true):
    ...
    result = result.sort_values(['proba', 'target'],
    ...
    result['lift'] = result['rate'] / (result['target'].sum() /
                                       result.shape[0])
    ...
    ax1.plot(result.index,
             [result['target'].sum() / result.shape[0]] * result.shape[0],
             color='blue',
             label='Lift random')
